...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Prompt {
            ...
            // In JSON mode, stdin is left available for interactive permission prompts
            // (e.g. the user piping 'y' to approve a bash escalation).  Consuming it
            // upfront as context would leave the prompter with empty stdin → auto-deny.
            let stdin_context = if output_format == CliOutputFormat::Json {
                None
            } else {
                read_piped_stdin()
            };
...
UU conflicted.rs",
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        // Legacy sessions live in the same fingerprinted sessions directory as new ones.
        let sessions_base = crate::sessions_dir().expect("sessions dir should resolve");
        let legacy_path = sessions_base.join("legacy.json");
        Session::new()
